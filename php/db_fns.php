<?php
    class Init {
        ...
        public function getTexts() {
            ...
            for($i = 0; $i < 9; $i++) {
                $sql = 'SELECT content FROM writing WHERE location = ' . $loc[$i];
            ...
            // the array's key consists of phase and location
            // i.e. 05 = phase 1, location 5
            // i.e. 22 = phase 3, location 2
            for($i = 0; $i < 9; $i++) {
                if($i < 3) {
                    $texts['0' . (string)$loc[$i]] = $text[$i];
                } else if($i < 6) {
                    $texts['1' . (string)$loc[$i]] = $text[$i];
                } else {
                    $texts['2' . (string)$loc[$i]] = $text[$i];
                }
            }
